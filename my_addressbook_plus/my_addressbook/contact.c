...
void add_people(struct con* c)
{
	...
	{
		struct inform* p;
		p = (struct inform*)realloc(c->s, (c->capacity + MAX)*sizeof(struct inform));//realloc只能用一次
		if (p == NULL)
		{
			printf("开辟失败\n");
		}
...
void empty_data(struct con* c)
{
	memset(c->s, 0, c->capacity);
	c->dex = 0;
}
...
void sort_people(struct con* c)
{
	...
	for (i = 0; i < (c->dex)-1; i++)
	{
		for (j = 0; j< (c->dex) - i - 1; j++)
		{
			if (c->s[j].age > c->s[j + 1].age)
			{
				tem = c->s[j];
				c->s[j] = c->s[j + 1];
				c->s[j + 1] = tem;
